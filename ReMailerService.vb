...
#End Region
    ...
    Protected Overrides Sub OnStart(ByVal args() As String)
        ...
        Try
            ...
            If TestKey Is Nothing Then
                ...
                objReg.WriteValue(objReg.HKeyLocalMachine, "Software\ReMailerService", "UserName", "SCANNER")
                objReg.WriteValue(objReg.HKeyLocalMachine, "Software\ReMailerService", "Password", "SCANNER")
                objReg.WriteValue(objReg.HKeyLocalMachine, "Software\ReMailerService", "DBName", "scanner")
            ...
            If cloudsvc_farm = "" Then cloudsvc_farm = "192.168.7.21"
            ...
            SqlS = "DELETE FROM scanner.dbo.MSGS_LOCK " & _
    ...
    Private Sub CheckEmailQueue()
        ...
        Try
            ...
            CurRecCnt = 0
            SqlS = "SELECT COUNT(*) FROM scanner.dbo.MSGS_LOCK WHERE MACHINE='" & MachineName & "'"
            If Debug = "Y" Then
            ...
            If CurRecCnt = 0 Then
                SqlS = "INSERT scanner.dbo.MSGS_LOCK " & _
                    ...
                    "AND NOT EXISTS (" & _
                    "SELECT MS_IDENT FROM scanner.dbo.MSGS_LOCK WHERE MS_IDENT=M.MS_IDENT" & _
                    ") " & _
            ...
            "FROM scanner.dbo.MSGS_LOCK WHERE MACHINE='" & MachineName & "'"
            If Debug = "Y" Then
            ...
            While dr.Read()
                ...
                Try
                    ...
                    wp = "http://cloudsvc.certegrity.com/basic/service.asmx/SendMail?sXML=" & HttpUtility.UrlEncode(wp)
                    If Debug = "Y" Then
            ...
            For i = 1 To LineNumber
                ...
                If uMS_IDENT(i, 2) <> "Error" Then
                    SqlS = "DELETE FROM scanner.dbo.MSGS_LOCK WHERE MS_IDENT=" & uMS_IDENT(i, 1)
                    If Debug = "Y" Then
                    ...
                    End Try
                Else
                    SqlS = "UPDATE scanner.dbo.MSGS_LOCK SET ATTEMPTS=ATTEMPTS+1 WHERE MS_IDENT=" & uMS_IDENT(i, 1)
                    If Debug = "Y" Then
            ...
            SqlS = "INSERT scanner.dbo.MSGS_DEAD " & _
                "(SEND_TO, SEND_FROM, SUBJECT, BODY, SENT_FLG, CREATED, SENT, " & _
                "ATTACHMENT, CC, BCC, HTML, TO_ID, SRC_TYPE, DEFER_UNTIL, EXPIRES, " & _
                ...
                "ATTACHMENT, CC, BCC, HTML, TO_ID, SRC_TYPE, DEFER_UNTIL, EXPIRES, " & _
                "ATTACH_DOC_ID, READ_FLG, READ_DT, FROM_NM, FROM_ID, SRC_ID, MS_IDENT, MACHINE, ATTACH_TYPE " & _
                "FROM scanner.dbo.MSGS_LOCK ML " & _
                "WHERE MACHINE='" & MachineName & "' AND ATTEMPTS>3 AND NOT EXISTS (" & _
                "SELECT MS_IDENT FROM scanner.dbo.MSGS_DEAD WHERE MS_IDENT=ML.MS_IDENT)"
            If Debug = "Y" Then
            ...
            SqlS = "UPDATE scanner.dbo.MESSAGES SET SENT_FLG='E' " & _
            "WHERE MS_IDENT IN (SELECT MS_IDENT FROM scanner.dbo.MSGS_LOCK " & _
            ...
            SqlS = "DELETE FROM scanner.dbo.MSGS_LOCK WHERE MACHINE='" & MachineName & "'  AND ATTEMPTS>3"
            If Debug = "Y" Then
